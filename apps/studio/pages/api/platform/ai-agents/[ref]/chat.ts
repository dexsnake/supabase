...
/**
 * Streaming SSE proxy for the agent-api chat endpoint.
 *
 * Uses the Edge Runtime (same as pages/api/ai/docs.ts) so that the SSE stream
 * from agent-api can be piped directly to the browser without buffering.
 *
 * Auth strategy:
 ...
 *   the studio's GoTrueClient already authenticated the session, and agent-api's own
 *   JWT_SECRET verification runs for direct API calls)
 * - Calls agent-api with the service role key + X-Agent-User-Id so agent-api has
...
export default async function handler(req: NextRequest) {
  ...
  const { ref } = req.nextUrl.pathname.match(
    /\/api\/platform\/ai-agents\/(?<ref>[^/]+)\/chat/
  )?.groups ?? {}
