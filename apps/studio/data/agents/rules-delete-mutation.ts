...
export async function deleteRule({ projectRef, id }: RuleDeleteVariables) {
  return requestAgentApi<{ success: boolean }>(`/api/platform/ai-agents/${projectRef}/rules/${id}`, {
    method: 'DELETE',
  })
}
...
export const useRuleDeleteMutation = () => {
  ...
  return useMutation({
    ...
    async onSuccess(_data, variables) {
      await queryClient.invalidateQueries({ queryKey: agentKeys.rules(variables.projectRef) })
