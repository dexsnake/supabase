...
export async function updateAlertResolution({
  ...
  return requestAgentApi(`/api/platform/ai-agents/${projectRef}/alerts/${id}`, {
    method: 'PATCH',
...
export const useAlertResolveMutation = () => {
  ...
  return useMutation({
    ...
    async onSuccess(_data, variables) {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: agentKeys.alerts(variables.projectRef) }),
        queryClient.invalidateQueries({ queryKey: agentKeys.alerts(variables.projectRef, false) }),
        queryClient.invalidateQueries({ queryKey: agentKeys.alerts(variables.projectRef, true) }),
      ])
