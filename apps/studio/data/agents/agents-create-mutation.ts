...
export async function createAgent({
  ...
  const response = await fetchPost(`/api/platform/ai-agents/${projectRef}/agents`, body)
...
export const useAgentCreateMutation = () => {
  ...
  return useMutation({
    ...
    async onSuccess(_data, variables) {
      await queryClient.invalidateQueries({ queryKey: agentKeys.list(variables.projectRef) })
