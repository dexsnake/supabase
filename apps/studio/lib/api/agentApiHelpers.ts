...
export function agentApiUrl(path: string, query?: URLSearchParams): string {
  const base = `${AGENT_API_URL}/api/${path}`
  const qs = query?.toString()
  return qs ? `${base}?${qs}` : base
}

export function agentApiHeaders(
  req: NextApiRequest,
  claims?: JwtPayload
...
async function handleUpstream(response: Response, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(502).json({ message: 'Invalid response from agent-api' })
  }
}
...
export async function proxyGet(
  ...
  try {
    const response = await fetch(agentApiUrl(path, qs.size ? qs : undefined), {
      headers: agentApiHeaders(req, claims),
    })
    return handleUpstream(response, res)
  } catch (err: any) {
    return res.status(502).json({ message: `Could not reach agent-api: ${err?.message}` })
  }
}
