...
- Your function code explicitly returned a 401 response
...
If the body contains a message you wrote, or nothing at all, then your function code _did_ execute and returned a 401 itself.

Go to: [Your function returned a 401](#your-function-returned-a-401)

### Case 3: Not sure

Run this query in [Log Explorer](/dashboard/project/_/logs/explorer?q=SELECT%0A++req.pathname+AS+function_name%2C%0A++CASE%0A++++WHEN+metadata.execution_id+IS+NOT+NULL%0A++++++THEN+%27your_code_returned_401%27%0A++++WHEN+metadata.execution_id+IS+NULL%0A++++++AND+%28new_authorization.prefix+IS+NOT+NULL%0A+++++++++++OR+payload.algorithm+<>+%27HS256%27%29%0A++++++THEN+%27incompatible_keys%27%0A++++WHEN+metadata.execution_id+IS+NULL%0A++++++AND+payload.role+IS+NOT+NULL%0A++++++THEN+%27invalid_key%27%0A++++WHEN+metadata.execution_id+IS+NULL%0A++++++AND+jwt_authorization+IS+NULL%0A++++++AND+new_authorization.prefix+IS+NULL%0A++++++THEN+%27missing_auth_header%27%0A++END+AS+cause%0AFROM%0A++function_edge_logs%0A++CROSS+JOIN+UNNEST%28metadata%29+AS+metadata%0A++CROSS+JOIN+UNNEST%28metadata.request%29+AS+req%0A++CROSS+JOIN+UNNEST%28metadata.response%29+AS+res%0A++LEFT+JOIN+UNNEST%28req.sb%29+AS+sb%0A++LEFT+JOIN+UNNEST%28sb.apikey%29+AS+apikey%0A++LEFT+JOIN+UNNEST%28apikey.authorization%29+AS+new_authorization%0A++LEFT+JOIN+UNNEST%28sb.jwt%29+AS+jwt%0A++LEFT+JOIN+UNNEST%28jwt.authorization%29+AS+jwt_authorization%0A++LEFT+JOIN+UNNEST%28jwt_authorization.payload%29+AS+payload%0AWHERE%0A++res.status_code+%3D+401%0ALIMIT+10%3B) to classify recent 401s:
...
SELECT
  ...
  CASE
    WHEN metadata.execution_id IS NOT NULL
      THEN 'your_code_returned_401'
    WHEN metadata.execution_id IS NULL
      AND (new_authorization.prefix IS NOT NULL
           OR payload.algorithm <> 'HS256')
      THEN 'incompatible_keys'
    WHEN metadata.execution_id IS NULL
      AND payload.role IS NOT NULL
      THEN 'invalid_key'
    WHEN metadata.execution_id IS NULL
      AND jwt_authorization IS NULL
      AND new_authorization.prefix IS NULL
      THEN 'missing_auth_header'
  END AS cause
FROM
  function_edge_logs
  ...
  LEFT JOIN UNNEST(apikey.authorization) AS new_authorization
  LEFT JOIN UNNEST(sb.jwt) AS jwt
  LEFT JOIN UNNEST(jwt.authorization) AS jwt_authorization
  LEFT JOIN UNNEST(jwt_authorization.payload) AS payload
WHERE
  res.status_code = 401
LIMIT 10;
```
...
The auth check passed (or is disabled). Your function ran, and somewhere in your code it's logic returned a 401.
...
2. Trace the condition that triggered it. If you're interacting with a third-party API in your code, that service may be returning 401 and you're forwarding it.
...
**Fix:** Disable the built-in JWT check using one of these methods:
...
The built-in check is enabled and the key you sent is properly formatted but doesn't match your project's keys.

**Fix (recommended):** Disable the built-in check using the steps in [Incompatible key format](#incompatible-key-format) above, and [handle auth in your function code](https://supabase.com/docs/guides/functions/auth).

**Fix (alternative):** If you want to keep the built-in check, ensure you're sending a valid key. Use one of your [legacy API keys](/dashboard/project/_/settings/api-keys/legacy) with the [Supabase client library](https://supabase.com/docs/guides/api/rest/client-libs) when making your request.
