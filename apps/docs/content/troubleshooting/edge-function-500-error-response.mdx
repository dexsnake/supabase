...
FROM function_logs AS console_logs
...
) AS invocation_events ON metadata.execution_id = invocation_events.execution_id 
WHERE 
  invocation_events.status_code = 500
    AND 
  metadata.level = 'error'
    AND
  metadata.event_type IN ('Log', 'UncaughtException')
    AND
  console_logs.event_message LIKE '%Error:%file:///%'
...
if (typeof user_submission === 'undefined') {
  return new Response(
    JSON.stringify({ message: 'Submission is empty. Please try again.' }),
...
When you customize the error response, it's important to define an approriate new message. It may also be worthwhile changing the default `500` code returned during errors to a value more reflective of the situation for easier debugging in the future:
...
A `SyntaxError` error occurs when Deno's grammatical rules are violated. For instance, failing to close parenthesis will cause the error to emerge:
...
Without the `OPTIONS` handler, requests from the browser will be misparsed, resulting in an `Unexpected end of JSON input` error log. 
