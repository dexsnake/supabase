...
chatRouter.post('/', async (c) => {
  ...
  if (appendOnlyMode) {
    ...
    try {
      if (alertId) {
        ...
        await sql`
          INSERT INTO agent.alert_messages (id, alert_id, user_id, agent_id, role, content)
        ...
        await sql`INSERT INTO agent.conversations (id, user_id, task_id, title) VALUES (${conversationId}, ${userId ?? null}, ${taskId ?? null}, ${title}) ON CONFLICT (id) DO NOTHING`
        await sql`
          INSERT INTO agent.conversation_messages (id, conversation_id, agent_id, task_id, role, parts)
  ...
  // Load agent
  const [agentRow] =
    await sql`SELECT id, name, summary, system_prompt, tools FROM agent.agents WHERE id = ${agentId}`
  ...
  if (alertId) {
    const msgRows = await sql`
      SELECT id, role, content, created_at FROM agent.alert_messages
    ...
    const convCheck =
      await sql`SELECT id, task_id FROM agent.conversations WHERE id = ${conversationId}`
  ...
  if (resolvedTaskId) {
    const [taskRow] =
      await sql`SELECT id, name, description, schedule, is_unique FROM agent.agent_tasks WHERE id = ${resolvedTaskId}`
  ...
  if (alertId) {
    const [alertRow] =
      await sql`SELECT title, message, severity, metadata FROM agent.alerts WHERE id = ${alertId}`
        ...
        try {
          ...
          if (alertId) {
            ...
            if (newAssistant.length > 0) {
              await sql`
                INSERT INTO agent.alert_messages ${sql(
                  newAssistant.map((m) => ({
                    id: m.id,
                    alert_id: alertId,
                    user_id: userId ?? null,
            ...
            if (!conversationExists) {
              ...
              await sql`INSERT INTO agent.conversations (id, user_id, task_id, title) VALUES (${conversationId}, ${userId ?? null}, ${resolvedTaskId ?? null}, ${title})`
            }
            ...
            if (newMessages.length > 0) {
              await sql`
                INSERT INTO agent.conversation_messages ${sql(
                  newMessages.map((m) => ({
                    id: m.id,
