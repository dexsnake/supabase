...
async function getAgentById(id: string) {
  const [row] = await sql`SELECT * FROM agent.agents WHERE id = ${id}`
  return row ?? null
}
...
apiRouter.get('/agents', async (c) => {
  const rows = await sql`SELECT * FROM agent.agents ORDER BY name`
  return c.json(rows)
})
...
apiRouter.get('/agents/:id', async (c) => {
  ...
  const [row] = await sql`
    SELECT
      ...
      (
        SELECT COUNT(*)::int
        FROM agent.alerts al
        WHERE al.agent_id = a.id
      ) AS alert_count,
      (
        SELECT COUNT(*)::int
        FROM agent.conversations conv
        WHERE
          EXISTS (
            SELECT 1
            FROM agent.agent_tasks task
          ...
          OR EXISTS (
            ...
            LEFT JOIN agent.agent_tasks task ON task.id = message.task_id
      ...
      (
        ...
        (
          SELECT COUNT(*)::int
          FROM agent.alert_messages message
          WHERE message.agent_id = a.id
        )
...
apiRouter.get('/agents/:id/logs', async (c) => {
  ...
  const conversationRows = await sql`
    ...
    FROM agent.conversation_messages message
    JOIN agent.conversations conv ON conv.id = message.conversation_id
    LEFT JOIN agent.agent_tasks conversation_task ON conversation_task.id = conv.task_id
    LEFT JOIN agent.agent_tasks message_task ON message_task.id = message.task_id
  ...
  const alertRows = await sql`
    ...
    FROM agent.alert_messages message
    JOIN agent.alerts alert ON alert.id = message.alert_id
    WHERE message.agent_id = ${id}
  `
...
apiRouter.post('/agents', async (c) => {
  ...
  const [row] = await sql`
    INSERT INTO agent.agents (name, summary, system_prompt, tools)
...
apiRouter.delete('/agents/:id', async (c) => {
  const id = c.req.param('id')
  await sql`DELETE FROM agent.agents WHERE id = ${id}`
...
apiRouter.get('/agent-tasks', async (c) => {
  const rows = await sql`SELECT * FROM agent.agent_tasks ORDER BY name`
  return c.json(rows)
})
...
apiRouter.post('/agent-tasks', async (c) => {
  ...
  const [row] = await sql`
    INSERT INTO agent.agent_tasks (user_id, agent_id, name, description, schedule, is_unique, enabled)
...
apiRouter.delete('/agent-tasks/:id', async (c) => {
  const id = c.req.param('id')
  const [row] = await sql`DELETE FROM agent.agent_tasks WHERE id = ${id} RETURNING id`
  if (!row) return c.json({ error: 'Not found' }, 404)
  return c.json({ success: true })
})
...
apiRouter.get('/conversations/:id/messages', async (c) => {
  const id = c.req.param('id')
  const check = await sql`SELECT id FROM agent.conversations WHERE id = ${id}`
  if (!check.length) return c.json({ error: 'Not found' }, 404)
  const rows = await sql`
    SELECT id, role, parts, created_at FROM agent.conversation_messages
...
apiRouter.get('/alerts', async (c) => {
  ...
  if (resolved === 'true') {
    rows =
      await sql`SELECT * FROM agent.alerts WHERE resolved_at IS NOT NULL ORDER BY created_at DESC`
  } else if (resolved === 'false') {
    rows = await sql`SELECT * FROM agent.alerts WHERE resolved_at IS NULL ORDER BY created_at DESC`
  } else {
    rows = await sql`SELECT * FROM agent.alerts ORDER BY created_at DESC`
  }
  return c.json(rows)
})
...
apiRouter.patch('/alerts/:id', async (c) => {
  const id = c.req.param('id')
  const body = await c.req.json()
  const check = await sql`SELECT id FROM agent.alerts WHERE id = ${id}`
  if (!check.length) return c.json({ error: 'Not found' }, 404)
  const resolvedAt = body.resolved_at ?? null
  const [row] = await sql`
    UPDATE agent.alerts SET resolved_at = ${resolvedAt} WHERE id = ${id} RETURNING *
  `
  if (resolvedAt) {
    await sql`
      UPDATE agent.alerts SET resolved_at = ${resolvedAt}
...
apiRouter.get('/alerts/:id/messages', async (c) => {
  ...
  const check = await sql`SELECT id FROM agent.alerts WHERE id = ${alertId}`
  if (!check.length) return c.json({ error: 'Not found' }, 404)
  const rows = await sql`
    SELECT * FROM agent.alert_messages WHERE alert_id = ${alertId} ORDER BY created_at ASC
  `
  return c.json(rows)
})
...
apiRouter.get('/rules', async (c) => {
  const rows = await sql`SELECT * FROM agent.rules ORDER BY title`
  return c.json(rows)
})
...
apiRouter.post('/rules', async (c) => {
  ...
  const [row] = await sql`
    INSERT INTO agent.rules (user_id, title, description, default_message, sql_query, edge_function_name, schedule, enabled)
...
apiRouter.delete('/rules/:id', async (c) => {
  const id = c.req.param('id')
  const [row] = await sql`DELETE FROM agent.rules WHERE id = ${id} RETURNING id`
  if (!row) return c.json({ error: 'Not found' }, 404)
  return c.json({ success: true })
})
