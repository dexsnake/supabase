...
async function refreshTask(id: string): Promise<void> {
  const [task] = await sql`SELECT id, schedule, enabled FROM agent.agent_tasks WHERE id = ${id}`
  if (task) {
...
async function refreshRule(id: string): Promise<void> {
  const [rule] = await sql`SELECT id, schedule, enabled FROM agent.rules WHERE id = ${id}`
  if (rule) {
...
export async function initScheduler(): Promise<void> {
  ...
  const [tasks, rules] = await Promise.all([
    sql`SELECT id, schedule, enabled FROM agent.agent_tasks`,
    sql`SELECT id, schedule, enabled FROM agent.rules`,
  ])
  ...
  await sql.listen('agent_scheduler', (payload) => {
    if (!payload) return
  ...
  console.log('[scheduler] listening on agent_scheduler channel')
}
