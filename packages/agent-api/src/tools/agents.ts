...
async function resolveAgentId(opts: {
  ...
  const [row] = await sql`
    SELECT agent_id FROM agent.agent_tasks
...
export const agentTools = {
  listAgents: tool({
    ...
    execute: async () => {
      const rows = await sql`SELECT id, name, summary, system_prompt, tools, created_at, updated_at FROM agent.agents ORDER BY name`
      return { agents: rows }
    },
  }),
  ...
  createAlert: tool({
    ...
    execute: async ({ title, message, severity, agent_id, parent_alert_id, conversation_message_id, rule_id, metadata }) => {
      ...
      const [row] = await sql`
        INSERT INTO agent.alerts (title, message, severity, agent_id, parent_alert_id, conversation_message_id, rule_id, user_id, metadata)
  ...
  getAlert: tool({
    ...
    execute: async ({ alert_id }) => {
      const [alert] = await sql`
        ...
        FROM agent.alerts WHERE id = ${alert_id}
      `
      if (!alert) return { error: 'Alert not found' }
      const messages = await sql`
        SELECT id, role, content, agent_id, created_at FROM agent.alert_messages
  ...
  listAlerts: tool({
    ...
    execute: async ({ resolved }) => {
      let rows
      if (resolved === true) {
        rows = await sql`SELECT * FROM agent.alerts WHERE resolved_at IS NOT NULL ORDER BY created_at DESC`
  ...
  commentOnAlert: tool({
    ...
    execute: async ({ alert_id, content, agent_id }) => {
      const ctx = getToolContext()
      const [check] = await sql`SELECT id FROM agent.alerts WHERE id = ${alert_id}`
  ...
  listTasks: tool({
    ...
    execute: async ({ agent_id }) => {
      ...
      const rows = await sql`
        ...
        FROM agent.agent_tasks WHERE user_id = ${ctx.userId} AND agent_id = ${effectiveAgentId}
