...
// Port of agent.cron_interval_seconds() from the prototype schema.
...
export async function runRule(ruleId: string): Promise<void> {
  ...
  const [rule] = (await sql`
    SELECT * FROM agent.rules WHERE id = ${ruleId} AND enabled = true
  `) as Rule[]
  ...
  if (rule.sql_query) {
    ...
    const [parentAlert] = await sql`
      SELECT a.id FROM agent.alerts a
      WHERE a.rule_id = ${ruleId}
        AND a.parent_alert_id IS NULL
        AND a.resolved_at IS NULL
        AND (
          SELECT MAX(created_at) FROM agent.alerts
    ...
    const [newAlert] = await sql`
      INSERT INTO agent.alerts (parent_alert_id, rule_id, title, message, severity, metadata)
      VALUES (
        ${parentAlert?.id ?? null},
        ${ruleId},
        ${rule.title},
